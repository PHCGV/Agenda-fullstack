...
export async function sendConfirmationEmail({ appointment, confirmUrl, cancelUrl, to, recipientName }) {
  const client = createClient();
  if (!client) {
    return { ok: false, error: "Resend is not configured" };
  }
  ...
  const when = formatDateTime(appointment.startAt);
  const subject = `Consolium | Confirmação de agendamento - ${when}`;
  const name = recipientName ?? (appointment.client && appointment.client.name) ?? "";
  const html = `
    <div style="background:#f6f2ed;padding:24px;font-family:'Space Grotesk',Arial,sans-serif;">
      <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #efe9e2;border-radius:18px;padding:24px;">
        <p style="margin:0 0 8px;color:#1f1b16;">Ola, ${name}.</p>
        <h2 style="margin:0 0 12px;font-size:20px;color:#1f1b16;">Confirmação de agendamento</h2>
        <p style="margin:0 0 12px;color:#6b645f;">O atendimento está agendado para <strong>${when}</strong>.</p>
        <p style="margin:0 0 20px;color:#6b645f;">Profissional: ${appointment.professional.name}</p>
        <div style="display:flex;gap:12px;margin-bottom:20px;">
          <a href="${confirmUrl}" style="background:#06b6d4;color:#ffffff;padding:10px 16px;text-decoration:none;border-radius:999px;font-weight:600;">Confirmar presença</a>
          <a href="${cancelUrl}" style="border:1px solid #f97316;color:#f97316;padding:10px 16px;text-decoration:none;border-radius:999px;font-weight:600;">Cancelar</a>
        </div>
        <p style="margin:0;font-size:12px;color:#6b645f;">Consolium</p>
      </div>
